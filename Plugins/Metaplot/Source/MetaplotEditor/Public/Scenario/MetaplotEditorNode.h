...
#include "MetaplotEditorNode.generated.h"

/**
 * Base for Evaluator, Task and Condition nodes.
 */
USTRUCT()
struct FMetaplotEditorNode
{
	GENERATED_BODY()
	 ...
	 */
	TStructView<FMetaplotEditorNode> GetNode() const
	{
		return TStructView<FMetaplotEditorNode>(Node.GetScriptStruct(), const_cast<uint8*>(Node.GetMemory()));
	}
...
template <typename T>
struct TMetaplotEditorNode : public FMetaplotEditorNode
{
	using NodeType = T;
