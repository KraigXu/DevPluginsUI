...
#include "MetaStorySchema.generated.h"
 ...
 * Each MetaStory asset saves the schema class name in asset data tags, which can be
 * used to limit which StatTree assets can be selected per use case, i.e.:
 *
 *	UPROPERTY(EditDefaultsOnly, Category = AI, meta=(RequiredAssetDataTags="Schema=MetaStorySchema_SupaDupa"))
 *	UMetaStory* MetaStory;
 *
 */
UCLASS(MinimalAPI, Abstract)
class UMetaStorySchema : public UObject
{
	GENERATED_BODY()
...
public:
	...
	virtual bool IsStateSelectionAllowed(EMetaStoryStateSelectionBehavior InBehavior) const
	{
		return true;
	}

	/** @return True if the state type is supported. */
	virtual bool IsStateTypeAllowed(EMetaStoryStateType InStateType) const
	{
		return true;
	}
	 ...
	 * @return True if the class is a MetaStory item Blueprint base class.
	 */
	...
	virtual TConstArrayView<FMetaStoryExternalDataDesc> GetContextDataDescs() const
	{
		return {};
	}

	/** @return the global parameter type used by the schema. */
	UE_API virtual EMetaStoryParameterDataType GetGlobalParameterDataType() const;

	/** @return the selection rules used by the schema. */
	UE_API virtual EMetaStoryStateSelectionRules GetStateSelectionRules() const;

	/** Resolves schema references to other MetaStory data. */
