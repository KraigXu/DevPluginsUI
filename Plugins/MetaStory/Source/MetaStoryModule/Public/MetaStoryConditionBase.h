...
#include "MetaStoryConditionBase.generated.h"

struct FMetaStoryExecutionContext;

enum class EMetaStoryCompare : uint8
{
	Default,
	Invert,
};
 ...
 */
USTRUCT(meta = (Hidden))
struct FMetaStoryConditionBase : public FMetaStoryNodeBase
{
	GENERATED_BODY()
	
	/** @return True if the condition passes. */
	virtual bool TestCondition(FMetaStoryExecutionContext& Context) const { return false; }
	 ...
	 */
	virtual void EnterState(FMetaStoryExecutionContext& Context, const FMetaStoryTransitionResult& Transition) const {}
	...
	UPROPERTY()
	EMetaStoryExpressionOperand Operand = EMetaStoryExpressionOperand::And;

	UPROPERTY()
	int8 DeltaIndent = 0;

	UPROPERTY()
	EMetaStoryConditionEvaluationMode EvaluationMode = EMetaStoryConditionEvaluationMode::Evaluated;
 ...
 */
USTRUCT(meta = (Hidden))
struct FMetaStoryConditionCommonBase : public FMetaStoryConditionBase
{
	GENERATED_BODY()
};

/** Helper macro to define instance data as simple constructible. */
#define METASTORY_POD_INSTANCEDATA(Type) \
