...
namespace UE::MetaStory::InstanceData
{
	...
	public:
		...
		using FMemoryRequirement = UE::MetaStory::InstanceData::FEvaluationScopeMemoryRequirement;
	...
	public:
		...
		[[nodiscard]] bool IsValidDataHandle(FMetaStoryDataHandle DataHandle) const
		{
			return Get(DataHandle) != nullptr;
		}

		/** @return specified item of an added structure. */
		[[nodiscard]] FMetaStoryDataView GetDataView(FMetaStoryDataHandle DataHandle)
		{
			return Get(DataHandle)->Instance;
		}

		/** @return specified item if the structure was added. */
		[[nodiscard]] FMetaStoryDataView* GetDataViewPtr(FMetaStoryDataHandle DataHandle)
		{
		 ...
		 */
		UE_API void Add(FMetaStoryDataHandle DataHandle, FConstStructView Default);
	...
	private:
		struct FItem
		{
			FMetaStoryDataView Instance;
			FMetaStoryDataHandle DataHandle;
		};

		FItem* Get(const FMetaStoryDataHandle& Handle) const;
	...
	private:
		/** Struct instances (Not transient, as we use FMetaStoryInstanceData to store default values for instance data) */
		void* Memory = nullptr;
		FMemoryRequirement MemoryRequirement;
