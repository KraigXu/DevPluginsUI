...
{
	/** A unique ID for FMetaStoryExecutionFrame. */
	...
	/** A unique ID for FMetaStoryActiveStates state. */
	...
	struct FActiveState
	{
		...
		inline explicit FActiveState(FActiveFrameID InFrameID, FActiveStateID InStateID, FMetaStoryStateHandle InHandle);
		...
		[[nodiscard]] FMetaStoryStateHandle GetStateHandle() const
		{
			return StateHandle;
		}
	...
	public:
		...
		[[nodiscard]] UE_API static TValueOrError<FString, void> Describe(TNotNull<const UMetaStory*> MetaStory, const TArrayView<const FActiveState>& Path);
	...
	// Inlined functions
	FActiveState::FActiveState(FActiveFrameID InFrameID, FActiveStateID InStateID, FMetaStoryStateHandle InHandle)
		: FrameID(InFrameID)
		, StateID(InStateID)
