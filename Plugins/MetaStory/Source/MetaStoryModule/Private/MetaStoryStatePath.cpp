...
namespace UE::MetaStory
{
	...
	TValueOrError<FString, void> FActiveStatePath::Describe(TNotNull<const UMetaStory*> MetaStory, const TArrayView<const FActiveState>& States)
	{
		const UMetaStory* CurrentTree = MetaStory;
		...
		for (const FActiveState& Element : States)
		{
...

			{
				...
				const FMetaStoryCompactState& State = CurrentTree->GetStates()[Element.GetStateHandle().Index];
				Builder << State.Name;
				Builder << TEXT('(');
	...
	{
		const UMetaStory* CurrentTree = MetaStory.Get();
		if (CurrentTree == nullptr)
		{
			return MakeError();
		}
