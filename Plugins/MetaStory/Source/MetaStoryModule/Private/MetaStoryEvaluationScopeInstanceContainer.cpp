...
{
	if (!Struct->IsChildOf(FMetaStoryInstanceObjectWrapper::StaticStruct()))
	{
...
void FEvaluationScopeInstanceContainer::Add(FMetaStoryDataHandle DataHandle, FConstStructView DefaultInstance)
{
	...
	if (!Struct->IsChildOf(FMetaStoryInstanceObjectWrapper::StaticStruct()))
	{
		...
		ItemContainer[NumberOfElements].Instance = FMetaStoryDataView(Struct, (uint8*)MemoryForNewItem);
	...
	else
	{
		...
		const FMetaStoryInstanceObjectWrapper& Wrapper = DefaultInstance.Get<const FMetaStoryInstanceObjectWrapper>();
		if (Wrapper.InstanceObject)
		{
			UObject* DuplicatedObject = ::DuplicateObject(Wrapper.InstanceObject, GetTransientPackage());

			ItemContainer[NumberOfElements].Instance = FMetaStoryDataView(DuplicatedObject);
			ItemContainer[NumberOfElements].DataHandle = DataHandle;
		}
	}
	++NumberOfElements;
...
void FEvaluationScopeInstanceContainer::Reset()
{
	...
	if (bStructsHaveDestructor)
	{
		...
		for (; NumberOfElements > 0; --NumberOfElements)
		{
			...
			{
				FMetaStoryDataView& Instance = ItemContainer[NumberOfElements - 1].Instance;
...
FEvaluationScopeInstanceContainer::FItem* FEvaluationScopeInstanceContainer::Get(const FMetaStoryDataHandle& DataHandle) const
{
	FItem* ItemContainer = static_cast<FItem*>(Memory);
...
{
#if WITH_METASTORY_DEBUG && DO_ENSURE
