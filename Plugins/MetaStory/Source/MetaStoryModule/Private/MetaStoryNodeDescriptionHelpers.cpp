...
FText GetOperatorText(const EGenericAICheck Operator, EMetaStoryNodeFormatting Formatting)
{
...
FText GetInvertText(bool bInvert, EMetaStoryNodeFormatting Formatting)
{
...
FText GetBoolText(bool bValue, EMetaStoryNodeFormatting Formatting)
{
	return bValue ? LOCTEXT("True", "True") : LOCTEXT("False", "False");
}

FText GetIntervalText(const FFloatInterval& Interval, EMetaStoryNodeFormatting Formatting)
{
	return GetIntervalText(Interval.Min, Interval.Max, Formatting);
}

FText GetIntervalText(float Min, float Max, EMetaStoryNodeFormatting Formatting)
{
...
FText GetIntervalText(const FText& MinValueText, const FText& MaxValueText, EMetaStoryNodeFormatting Formatting)
{
	FText IntervalText;
	if (Formatting == EMetaStoryNodeFormatting::RichText)
	{
...
FText GetExactMatchText(bool bExactMatch, EMetaStoryNodeFormatting Formatting)
{
	if (Formatting == EMetaStoryNodeFormatting::RichText)
	{
...
FText GetText(const FVector& Value, EMetaStoryNodeFormatting Formatting)
{
	return Value.ToCompactText();
}

FText GetText(float Value, EMetaStoryNodeFormatting Formatting)
{
	return FText::AsNumber(Value);
}

FText GetText(int32 Value, EMetaStoryNodeFormatting Formatting)
{
	return FText::AsNumber(Value);
}

FText GetText(const UObject* Value, EMetaStoryNodeFormatting Formatting)
{
	return FText::FromName(GetFNameSafe(Value));
}

FText GetMathOperationText(const FText& OperationText, const FText& LeftValue, const FText& RightValue, EMetaStoryNodeFormatting Formatting)
{
...
FText GetSingleParamFunctionText(const FText& FunctionText, const FText& ParamText, EMetaStoryNodeFormatting Formatting)
{
	const FText Format = (Formatting == EMetaStoryNodeFormatting::RichText)
