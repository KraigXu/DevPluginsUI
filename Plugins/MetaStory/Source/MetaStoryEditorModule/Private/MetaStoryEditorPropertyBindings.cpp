...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MetaStoryEditorPropertyBindings)

#define LOCTEXT_NAMESPACE "MetaStoryEditor"

UMetaStoryEditorPropertyBindingsOwner::UMetaStoryEditorPropertyBindingsOwner(const FObjectInitializer& ObjectInitializer)
...
FPropertyBindingPath FMetaStoryEditorPropertyBindings::AddFunctionBinding(const UScriptStruct* InPropertyFunctionNodeStruct, TConstArrayView<FPropertyBindingPathSegment> InSourcePathSegments, const FPropertyBindingPath& TargetPath)
{
	check(InPropertyFunctionNodeStruct->IsChildOf<FMetaStoryPropertyFunctionBase>());

	FInstancedStruct PropertyFunctionNode(FMetaStoryEditorNode::StaticStruct());
	FMetaStoryEditorNode& PropertyFunction = PropertyFunctionNode.GetMutable<FMetaStoryEditorNode>();
	...
	const FMetaStoryPropertyFunctionBase& Function = PropertyFunction.Node.Get<FMetaStoryPropertyFunctionBase>();
...
const FMetaStoryPropertyPathBinding* FMetaStoryEditorPropertyBindings::AddOutputBinding(const FPropertyBindingPath& InSourcePath,
	const FPropertyBindingPath& InTargetPath)
{
	if (FMetaStoryPropertyPathBinding* AddedBinding = static_cast<FMetaStoryPropertyPathBinding*>(AddBindingInternal(InSourcePath, InTargetPath)))
	{
...
FPropertyBindingBinding* FMetaStoryEditorPropertyBindings::AddBindingInternal(const FPropertyBindingPath& InSourcePath
	, const FPropertyBindingPath& InTargetPath)
{
...
void FMetaStoryEditorPropertyBindings::CopyBindingsInternal(const FGuid InFromStructID, const FGuid InToStructID)
{
		...
		CopyBindingsImplementation(FromStructID, ToStructID, [ToStructID, &FunctionBindingsToCopy](const FPropertyBindingBinding& Binding)
			{
				...
				{
					const FMetaStoryEditorNode* EditorNode = Binding.GetPropertyFunctionNode().GetPtr<const FMetaStoryEditorNode>();
...
void FMetaStoryEditorPropertyBindings::RemoveBindingsInternal(TFunctionRef<bool(FPropertyBindingBinding&)> InPredicate)
{
	PropertyBindings.RemoveAllSwap(InPredicate);
}

bool FMetaStoryEditorPropertyBindings::HasBindingInternal(TFunctionRef<bool(const FPropertyBindingBinding&)> InPredicate) const
{
	return PropertyBindings.ContainsByPredicate(InPredicate);
}

const FPropertyBindingBinding* FMetaStoryEditorPropertyBindings::FindBindingInternal(TFunctionRef<bool(const FPropertyBindingBinding&)> InPredicate) const
{
	return PropertyBindings.FindByPredicate(InPredicate);
}

void FMetaStoryEditorPropertyBindings::ForEachBinding(TFunctionRef<void(const FPropertyBindingBinding& Binding)> InFunction) const
{
	for (const FMetaStoryPropertyPathBinding& Binding : PropertyBindings)
	{
		InFunction(Binding);
	}
}

void FMetaStoryEditorPropertyBindings::ForEachBinding(const FPropertyBindingIndex16 InBegin, const FPropertyBindingIndex16 InEnd
...
void FMetaStoryEditorPropertyBindings::ForEachMutableBinding(TFunctionRef<void(FPropertyBindingBinding& Binding)> InFunction)
{
	for (FMetaStoryPropertyPathBinding& Binding : PropertyBindings)
	{
		InFunction(Binding);
	}
}

void FMetaStoryEditorPropertyBindings::VisitBindings(TFunctionRef<EVisitResult(const FPropertyBindingBinding& Binding)> InFunction) const
{
	for (const FMetaStoryPropertyPathBinding& Binding : PropertyBindings)
	{
...
void FMetaStoryEditorPropertyBindings::VisitMutableBindings(TFunctionRef<EVisitResult(FPropertyBindingBinding& Binding)> InFunction)
{
	for (FMetaStoryPropertyPathBinding& Binding : PropertyBindings)
	{
		if (InFunction(Binding) == EVisitResult::Break)
...
const FPropertyBindingBindableStructDescriptor* FMetaStoryEditorPropertyBindings::GetBindableStructDescriptorFromHandle(
	FConstStructView InSourceHandleView) const
{
	// This is not used for Editor operation and handled at runtime in FMetaStoryPropertyBindings
	return nullptr;
}

int32 FMetaStoryEditorPropertyBindings::GetNumBindableStructDescriptors() const
{
	// This is not used for Editor operation and handled at runtime in FMetaStoryPropertyBindings
	return 0;
}

int32 FMetaStoryEditorPropertyBindings::GetNumBindings() const
{
...
FMetaStoryBindingLookup::FMetaStoryBindingLookup(const IMetaStoryEditorPropertyBindingsOwner* InBindingOwner)
	: BindingOwner(InBindingOwner)
{
}

const FPropertyBindingPath* FMetaStoryBindingLookup::GetPropertyBindingSource(const FPropertyBindingPath& InTargetPath) const
{
	check(BindingOwner);
	const FMetaStoryEditorPropertyBindings* EditorBindings = BindingOwner->GetPropertyEditorBindings();
...
FText FMetaStoryBindingLookup::GetPropertyPathDisplayName(const FPropertyBindingPath& InPath, EMetaStoryNodeFormatting Formatting) const
{
	check(BindingOwner);
	const FMetaStoryEditorPropertyBindings* EditorBindings = BindingOwner->GetPropertyEditorBindings();
	check(EditorBindings);
	...
	// If path's struct is a PropertyFunction, let it override a display name.
	{
		...
		if (BindingToPath && BindingToPath->GetPropertyFunctionNode().IsValid())
		{
			...
			const FMetaStoryEditorNode& EditorNode = PropertyFuncEditorNodeView.Get<const FMetaStoryEditorNode>();
			...
			const FMetaStoryNodeBase& Node = EditorNode.Node.Get<FMetaStoryNodeBase>();

			// Skipping an output property if there's only one of them.
			if (UE::MetaStory::GetStructSingleOutputProperty(*Node.GetInstanceDataType()))
			{
...
FText FMetaStoryBindingLookup::GetBindingSourceDisplayName(const FPropertyBindingPath& InTargetPath, EMetaStoryNodeFormatting Formatting) const
{
	check(BindingOwner);
	const FMetaStoryEditorPropertyBindings* EditorBindings = BindingOwner->GetPropertyEditorBindings();
	check(EditorBindings);
	...
	if (static_cast<const IPropertyBindingBindingCollectionOwner*>(BindingOwner)->GetBindableStructByID(InTargetPath.GetStructID(), TargetStructDesc))
	{
			...
			&& Indirection.Num() > 0)
		{
			...
			if (TargetProperty)
			{
				Usage = UE::MetaStory::GetUsageFromMetaData(TargetProperty);
			}
	...
	if (Usage == EMetaStoryPropertyUsage::Context)
	{
		if (TargetStruct)
		{
			const FMetaStoryBindableStructDesc Desc = BindingOwner->FindContextData(TargetStruct, TargetProperty->GetName());
			if (Desc.IsValid())
			{
				// Connected
...
const FProperty* FMetaStoryBindingLookup::GetPropertyPathLeafProperty(const FPropertyBindingPath& InPath) const
{
	check(BindingOwner);
	const FMetaStoryEditorPropertyBindings* EditorBindings = BindingOwner->GetPropertyEditorBindings();
	check(EditorBindings);
