...
struct FGuid;
struct FSlateBrush;
struct FMetaStoryEditPropertyPath;
 ...
 */
void DispatchPostEditToNodes(UObject& Owner, FPropertyChangedChainEvent& PropertyChangedEvent, UMetaStoryEditorData& EditorData);

/**
 * Modify a MetaStoryState in PreEdit and PostEdit callbacks in a transaction.
 */
void ModifyStateInPreAndPostEdit(
	...
	TFunctionRef<void(TNotNull<UMetaStoryState*>, TNotNull<UMetaStoryEditorData*>, const FMetaStoryEditPropertyPath&)> Func,
...
{
private:
	struct FMetaStoryEditPropertySegment
	{
		FMetaStoryEditPropertySegment() = default;
		FMetaStoryEditPropertySegment(FProperty* InProperty, const FName InPropertyName, const int32 InArrayIndex = INDEX_NONE)
			: Property(InProperty)
...
public:
	FMetaStoryEditPropertyPath() = default;

	/** Makes property path relative to BaseStruct. Checks if the path is not part of the type. */
	explicit FMetaStoryEditPropertyPath(const UStruct* BaseStruct, FStringView InPath);

	/** Makes property path from property change event. */
	explicit FMetaStoryEditPropertyPath(const FPropertyChangedChainEvent& PropertyChangedEvent);

	/** Makes property path from property chain. */
	explicit FMetaStoryEditPropertyPath(const FEditPropertyChain& PropertyChain);
	...
	bool ContainsPath(const FMetaStoryEditPropertyPath& InPath) const;

	/** @return true if the property path is exactly the specified path. */
	bool IsPathExact(const FMetaStoryEditPropertyPath& InPath) const;

	/** @return array index at specified property, or INDEX_NONE, if the property is not array or property not found.  */
	int32 GetPropertyArrayIndex(const FMetaStoryEditPropertyPath& InPath) const
	{
		return ContainsPath(InPath) ? Path[InPath.Path.Num() - 1].ArrayIndex : INDEX_NONE;
	}

private:
	TArray<FMetaStoryEditPropertySegment> Path;
};
