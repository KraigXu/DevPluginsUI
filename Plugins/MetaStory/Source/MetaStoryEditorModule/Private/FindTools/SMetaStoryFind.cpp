...
#define LOCTEXT_NAMESPACE "MetaStoryFindInAsset"
...
void SFindInAsset::Construct(const FArguments& InArgs, TSharedPtr<IMetaStoryEditorHost> InEditorHost)
{
...
void SFindInAsset::HandleCopyAction()
{
	...
	for (const TSharedPtr<FFindResult>& SelectedItem : SelectedItems)
	{
		...
		{
		case FFindResult::EResultType::MetaStory:
			if (const UMetaStory* MetaStory = SelectedItem->MetaStory.Get())
			{
				MetaStory->GetPathName(nullptr, CopyText);
			}
			break;
		...
		if (UMetaStory* MetaStoryPtr = Cast<UMetaStory>(MetaStorysToProcess.Pop().ResolveObjectPtr()))
		{
			...
			auto ProcessProperty = [Self = this, &StackItems, MetaStoryPtr](const TPropertyValueIterator<FProperty>& Iterator, const FString& Value)
			{
				...
				if (bMatches)
				{
					...
					// Find the root
					TSharedPtr<FFindResult>* CurrentParentNode = Self->ItemsFound.FindByPredicate([MetaStoryPtr](const TSharedPtr<FFindResult>& Item)
					{
						check(Item->Type == FFindResult::EResultType::MetaStory);
						return Item->MetaStory == MetaStoryPtr;
					});
					if (!CurrentParentNode)
					{
						CurrentParentNode = &(Self->ItemsFound.Add_GetRef(MakeShared<FFindResult>()));
						(*CurrentParentNode)->Type = FFindResult::EResultType::MetaStory;
						(*CurrentParentNode)->Name = MetaStoryPtr->GetFName();
						(*CurrentParentNode)->MetaStory = MetaStoryPtr;
					}

					// Start the index at one to skip the EditorData object
					for (int32 Index = 1; Index < StackItems.Num(); ++Index)
					{
						auto CreateNode = [&CurrentParentNode, MetaStoryPtr](FGuid ID, FName Name, const FSlateBrush* Brush, FSlateColor Color, FFindResult::EResultType ResultType)
						{
							// Find the owner
						...
						if (StackItems[Index].Struct == FMetaStoryEditorNode::StaticStruct())
						{
							const FMetaStoryEditorNode* EditorNode = reinterpret_cast<const FMetaStoryEditorNode*>(StackItems[Index].Data);
							check(EditorNode);
							const FSlateBrush* Brush = nullptr;
							FSlateColor BrushColor = FSlateColor::UseForeground();
							if (const FMetaStoryNodeBase* BaseNode = EditorNode->Node.GetPtr<const FMetaStoryNodeBase>())
							{
								Brush = UE::MetaStoryEditor::EditorNodeUtils::ParseIcon(BaseNode->GetIconName()).GetIcon();
						...
						else if (StackItems[Index].Struct == UMetaStoryState::StaticClass())
						{
							const UMetaStoryState* State = reinterpret_cast<const UMetaStoryState*>(StackItems[Index].Data);
							check(State);
							const FSlateBrush* Brush = FMetaStoryEditorStyle::GetBrushForSelectionBehaviorType(State->SelectionBehavior, !State->Children.IsEmpty(), State->Type);
				...
				// Add linked MetaStory
				if (const FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Property))
				{
					if (const UMetaStoryState* State = Cast<const UMetaStoryState>(ObjectProperty->GetObjectPropertyValue(Iterator.Value())))
					{
			...
			Private::IterateProperties(StackItems, MetaStoryPtr->EditorData->GetClass(), MetaStoryPtr->EditorData, ProcessProperty);
		}
	}

	if (MetaStorysToProcess.IsEmpty())
	{
		bSearching = false;
...
{
	if (InItem->Type == FFindResult::EResultType::MetaStory)
	{
...
void SFindInAsset::HandleTreeSelectionDoubleClicked(TSharedPtr<FFindResult> Item)
{
	if (Item.IsValid())
	{
		TSharedPtr<FFindResult> MetaStoryParent = Item;
		FGuid NodeID;
		FGuid StateID;
		while (MetaStoryParent && MetaStoryParent->Type != FFindResult::EResultType::MetaStory)
		{
			if (MetaStoryParent->Type == FFindResult::EResultType::Node && !NodeID.IsValid())
			{
				NodeID = MetaStoryParent->ID;
			}
			if (MetaStoryParent->Type == FFindResult::EResultType::State && !StateID.IsValid())
			{
				StateID = MetaStoryParent->ID;
			}
			MetaStoryParent = MetaStoryParent->Parent.Pin();
		}

		if (UMetaStory* MetaStory = MetaStoryParent ? MetaStoryParent->MetaStory.Get() : nullptr)
		{
			if (UMetaStoryEditingSubsystem* MetaStoryEditingSubsystem = GEditor->GetEditorSubsystem<UMetaStoryEditingSubsystem>())
			{
				TSharedRef<FMetaStoryViewModel> ViewModel = MetaStoryEditingSubsystem->FindOrAddViewModel(MetaStory);
