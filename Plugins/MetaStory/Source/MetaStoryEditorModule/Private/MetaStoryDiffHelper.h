...
struct FStateSoftPath
{
	...
	explicit FStateSoftPath(TNotNull<const UMetaStoryState*> InState);

	UMetaStoryState* ResolvePath(TNotNull<const UMetaStory*> MetaStory) const;
	UMetaStoryState* ResolvePath(TNotNull<const UMetaStoryEditorData*> EditorData) const;
...
private:
	void PostOrderChain(const UMetaStoryState* CurBackwardsState);
	...
	struct FChainElement
	{
		...
		explicit FChainElement(const UMetaStoryState* State);
...
FText GetMetaStoryDiffMessage(const FSingleDiffEntry& Difference, FText ObjectName, bool bShort = false);
