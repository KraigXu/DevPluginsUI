...
namespace UE::MetaStory::PropertyBinding
{

/**
 * Information for the types gathered from a FMetaStoryPropertyRef property meta-data 
...
UObject* FindEditorBindingsOwner(UObject* InObject)
{
	...
	{
		IMetaStoryEditorPropertyBindingsOwner* BindingOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(Outer);
		if (BindingOwner)
		{
			Result = Outer;
			break;
...
UObject* FindStateOrEditorDataOwner(UObject* InObject)
{
	...
	for (UObject* Outer = InObject; Outer; Outer = Outer->GetOuter())
	{
		if (Outer->IsA<UMetaStoryEditorData>() || Outer->IsA<UMetaStoryState>())
		{
			Result = Outer;
			break;
		}
...
static bool IsDelegateListenerProperty(const FProperty* InProperty)
{
	if (const FStructProperty* StructProperty = CastField<FStructProperty>(InProperty))
	{
		return StructProperty->Struct == FMetaStoryDelegateListener::StaticStruct() || StructProperty->Struct == FMetaStoryTransitionDelegateListener::StaticStruct();
	}
...
static bool IsNodeStructProperty(const TSharedPtr<const IPropertyHandle>& InPropertyHandle)
{
	...
	if (InPropertyHandle && InPropertyHandle->GetValueData(Address) == FPropertyAccess::Success)
	{
		...
		if (Address && StructProperty)
		{
			...
			if (Struct)
			{
				return Struct->IsChildOf<FMetaStoryNodeBase>();
			}
		}
	}
...
FOnMetaStoryPropertyBindingChanged METASTORYEDITORMODULE_API OnMetaStoryPropertyBindingChanged;

struct FMetaStoryCachedBindingData : UE::PropertyBinding::FCachedBindingData, TSharedFromThis<FMetaStoryCachedBindingData>
{
	friend FMetaStoryBindingExtension;

	using Super = FCachedBindingData;

	FMetaStoryCachedBindingData(IPropertyBindingBindingCollectionOwner* InPropertyBindingsOwner, const FPropertyBindingPath& InTargetPath,
	...
	{
		const FMetaStoryBindableStructDesc& SourceDesc = InDescriptor.Get<FMetaStoryBindableStructDesc>();
		IMetaStoryEditorPropertyBindingsOwner* BindingsOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(GetOwner());
		FMetaStoryEditorPropertyBindings* EditorBindings = BindingsOwner->GetPropertyEditorBindings();

		if (SourceDesc.DataSource == EMetaStoryBindableStructSource::PropertyFunction)
		{
			const UScriptStruct* PropertyFunctionNodeStruct = nullptr;

			BindingsOwner->EnumerateBindablePropertyFunctionNodes([ID = SourceDesc.ID, &PropertyFunctionNodeStruct](const UScriptStruct* NodeStruct, const FMetaStoryBindableStructDesc& Desc, const FPropertyBindingDataView Value)
				{
					if (Desc.ID == ID)
					{
			...
			if (ensure(PropertyFunctionNodeStruct))
			{
				...
				{
					const FProperty* SingleOutputProperty = UE::MetaStory::GetStructSingleOutputProperty(*SourceDesc.Struct);
		...
		{
			if (ensureMsgf(SourceDesc.DataSource == EMetaStoryBindableStructSource::Parameter 
				|| SourceDesc.DataSource == EMetaStoryBindableStructSource::StateParameter, 
	...
	virtual void UpdateSourcePropertyPath(const TConstStructView<FPropertyBindingBindableStructDescriptor> InDescriptor, const FPropertyBindingPath& InSourcePath, FString& OutString) override
	{
		...
		if (SourceDesc.DataSource == EMetaStoryBindableStructSource::PropertyFunction && GetStructSingleOutputProperty(*SourceDesc.Struct))
		{
	...
	virtual void AddPropertyInfoOverride(const FProperty& Property, TArray<TSharedPtr<const UE::PropertyBinding::FPropertyInfoOverride>>& OutPropertyInfoOverrides) const override
	{
		...
		if (StructProperty && StructProperty->Struct && StructProperty->Struct->IsChildOf(FMetaStoryPropertyRef::StaticStruct()))
		{
			TArray<FEdGraphPinType, TInlineAllocator<1>> PinTypes;
			if (StructProperty->Struct->IsChildOf(FMetaStoryBlueprintPropertyRef::StaticStruct()))
			{
				...
				if (GetPropertyHandle()->GetValueData(PropertyRefAddress) == FPropertyAccess::Result::Success)
				{
					...
					PinTypes.Add(PropertyRefHelpers::GetBlueprintPropertyRefInternalTypeAsPin(*static_cast<const FMetaStoryBlueprintPropertyRef*>(PropertyRefAddress)));
				}
			}
			else
			{
	...
	virtual bool CanBindToContextStructInternal(const UStruct* InStruct, const int32 InStructIndex) override
	{
		...
		// Do not allow to bind directly MetaStory nodes
		// @todo: find a way to more specifically call out the context structs, e.g. pass the property path to the callback.
		if (InStruct != nullptr)
		{
			const bool bIsMetaStoryNode = GetAccessibleStructs().ContainsByPredicate([InStruct](const TInstancedStruct<FPropertyBindingBindableStructDescriptor>& StructDesc)
			{
				const FMetaStoryBindableStructDesc& AccessibleStruct = StructDesc.Get<FMetaStoryBindableStructDesc>();
				return AccessibleStruct.DataSource != EMetaStoryBindableStructSource::Context
					&& AccessibleStruct.DataSource != EMetaStoryBindableStructSource::Parameter
					&& AccessibleStruct.DataSource != EMetaStoryBindableStructSource::TransitionEvent
					&& AccessibleStruct.DataSource != EMetaStoryBindableStructSource::StateEvent
					&& AccessibleStruct.DataSource != EMetaStoryBindableStructSource::PropertyFunction
		...
		const FMetaStoryBindableStructDesc& StructDesc = GetBindableStructDescriptor(InStructIndex).Get<FMetaStoryBindableStructDesc>();
		// Binding directly into PropertyFunction's struct is allowed if it contains a compatible single output property.
		if (StructDesc.DataSource == EMetaStoryBindableStructSource::PropertyFunction)
		{
			const IMetaStoryEditorPropertyBindingsOwner* BindingOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(GetOwner());
			FMetaStoryDataView DataView;
			...
			if (const FStructProperty* StructProperty = CastField<FStructProperty>(GetPropertyHandle()->GetProperty()))
			{
				...
				if (StructProperty->Struct && StructProperty->Struct->IsChildOf<FMetaStoryPropertyRef>())
				{
					return false;
				}
			}
	...
	virtual bool CanAcceptPropertyOrChildrenInternal(const FProperty& SourceProperty, TConstArrayView<FBindingChainElement, int> InBindingChain) override
	{
		...
		const FMetaStoryBindableStructDesc& StructDesc = GetBindableStructDescriptor(SourceStructIndex).Get<FMetaStoryBindableStructDesc>();

		const FProperty* TargetProperty = GetPropertyHandle()->GetProperty();

		if (StructDesc.DataSource == EMetaStoryBindableStructSource::PropertyFunction)
		{
			IMetaStoryEditorPropertyBindingsOwner* BindingOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(GetOwner());
			FMetaStoryDataView DataView;
			// If DataView exists, struct is an instance of already bound function.
			if (BindingOwner == nullptr || BindingOwner->GetBindingDataViewByID(StructDesc.ID, DataView))
			{
			...
			if (InBindingChain.Num() == 1 && GetUsageFromMetaData(&SourceProperty) != EMetaStoryPropertyUsage::Output)
			{
				return false;
			}
		}
	...
	{
		// @TODO: Refactor FMetaStoryPropertyBindings::ResolveCopyType() so that we can use it directly here.
		...
		if (IsOutputBinding())
		{
			...
			if (CheckInvalidStructTypes.operator()<FMetaStoryDelegateDispatcher, FMetaStoryDelegateListener, FMetaStoryPropertyRef>(InSourceProperty) 
				|| CheckInvalidStructTypes.operator()<FMetaStoryDelegateDispatcher, FMetaStoryDelegateListener, FMetaStoryPropertyRef>(InTargetProperty))
			{
				return true;
			}
		...
		if (TargetStructProperty && TargetStructProperty->Struct == FMetaStoryAnyEnum::StaticStruct())
		{
			...
			const FMetaStoryAnyEnum* TargetAnyEnum = static_cast<const FMetaStoryAnyEnum*>(InTargetPropertyValue);
		...
		else if (TargetStructProperty && TargetStructProperty->Struct == FMetaStoryStructRef::StaticStruct())
		{
	...
	{
		const FMetaStoryEditorNode& EditorNode = InPropertyFunctionStructView.Get<const FMetaStoryEditorNode>();
		if (const FMetaStoryNodeBase* Node = EditorNode.Node.GetPtr<const FMetaStoryNodeBase>())
		{
			IMetaStoryEditorPropertyBindingsOwner* BindingOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(GetOwner());
			const FText Description = Node->GetDescription(GetSourcePath().GetStructID(), EditorNode.GetInstance(), FMetaStoryBindingLookup(BindingOwner), EMetaStoryNodeFormatting::Text);
	...
	virtual bool GetPropertyFunctionImage(FConstStructView InPropertyFunctionStructView, const FSlateBrush*& OutImage) const override
	{
		...
		if (const FMetaStoryNodeBase* Node = EditorNode.Node.GetPtr<const FMetaStoryNodeBase>())
		{
			if (GetStructSingleOutputProperty(*Node->GetInstanceDataType()))
			{
				OutImage = UE::MetaStoryEditor::EditorNodeUtils::ParseIcon(Node->GetIconName()).GetIcon();
				return true;
			}
		}
		return false;
...
private:
	...
	{
		return HierarchicalPropertyUsage == EMetaStoryPropertyUsage::Output;
	}

	bool IsPropertyAccessibleForOutputBinding(TConstArrayView<FBindingChainElement> InSourceBindingChain) const
	{
		const FMetaStoryBindableStructDesc& SourceStructDesc = GetAccessibleStructs()[InSourceBindingChain[0].ArrayIndex].Get<FMetaStoryBindableStructDesc>();
	...
	EMetaStoryPropertyUsage HierarchicalPropertyUsage = EMetaStoryPropertyUsage::Invalid;
};

/* Provides PropertyFunctionNode instance for a property node. */
class FMetaStoryPropertyFunctionNodeProvider : public IStructureDataProvider
{
public:
	FMetaStoryPropertyFunctionNodeProvider(IMetaStoryEditorPropertyBindingsOwner& InBindingsOwner, FPropertyBindingPath InTargetPath)
	...
	{
		return FMetaStoryEditorNode::StaticStruct();
	}
...
private:
	...
	{
		if(IMetaStoryEditorPropertyBindingsOwner* Owner = Cast<IMetaStoryEditorPropertyBindingsOwner>(RawBindingsOwner))
		{
			FMetaStoryEditorPropertyBindings* EditorBindings = Owner->GetPropertyEditorBindings();
			FPropertyBindingBinding* FoundBinding = EditorBindings->GetMutableBindings().FindByPredicate([&InTargetPath](const FMetaStoryPropertyPathBinding& Binding)
			{
			...
			if (FoundBinding)
			{
				...
				{
					const FMetaStoryEditorNode& EditorNode = EditorNodeView.Get<FMetaStoryEditorNode>();
...
} // UE::MetaStory::PropertyBinding

TSharedPtr<UE::PropertyBinding::FCachedBindingData> FMetaStoryBindingExtension::CreateCachedBindingData(IPropertyBindingBindingCollectionOwner* InBindingsOwner, const FPropertyBindingPath& InTargetPath, const TSharedPtr<IPropertyHandle>& InPropertyHandle, const TConstArrayView<TInstancedStruct<FPropertyBindingBindableStructDescriptor>> InAccessibleStructs) const
{
	return MakeShared<UE::MetaStory::PropertyBinding::FMetaStoryCachedBindingData>(InBindingsOwner, InTargetPath, InPropertyHandle, InAccessibleStructs);
}

bool FMetaStoryBindingExtension::CanBindToProperty(const FPropertyBindingPath& InTargetPath, const IPropertyHandle& InPropertyHandle) const
{
	...
	const bool bIsPropertyRef = UE::MetaStory::PropertyRefHelpers::IsPropertyRef(*PropertyToBind);
	const bool bIsDelegateListener = UE::MetaStory::PropertyBinding::IsDelegateListenerProperty(PropertyToBind);
	...
	if (!bIsPropertyRef && !bIsDelegateListener)
	{
		...
		{
			if (UE::MetaStory::PropertyBinding::IsNodeStructProperty(CurrentPropertyHandle))
			{
				return false;
			}
	...
	const EMetaStoryPropertyUsage Usage = UE::MetaStory::GetUsageFromMetaData(InPropertyHandle.GetProperty());
	if (Usage == EMetaStoryPropertyUsage::Input || Usage == EMetaStoryPropertyUsage::Context)
	{
		// Allow to bind only to the main level on input and context properties.
		return InTargetPath.GetSegments().Num() == 1;
	}

	return Usage == EMetaStoryPropertyUsage::Parameter || Usage == EMetaStoryPropertyUsage::Output;
}

void FMetaStoryBindingExtension::UpdateContextStruct(TConstStructView<FPropertyBindingBindableStructDescriptor> InStructDesc, FBindingContextStruct& InOutContextStruct, TMap<FString, FText>& InOutSectionNames) const
{
	const FMetaStoryBindableStructDesc& StructDesc = InStructDesc.Get<FMetaStoryBindableStructDesc>();
	...
	if (StructDesc.DataSource == EMetaStoryBindableStructSource::PropertyFunction)
	{
		if (const FProperty* OutputProperty = UE::MetaStory::GetStructSingleOutputProperty(*StructDesc.Struct))
		{
...
bool FMetaStoryBindingExtension::GetPromotionToParameterOverrideInternal(const FProperty& InProperty, bool& bOutOverride) const
{
	if (const FStructProperty* StructProperty = CastField<FStructProperty>(&InProperty))
	{
		...
		if (StructProperty->Struct && StructProperty->Struct->IsChildOf(FMetaStoryPropertyRef::StaticStruct()))
		{
			bOutOverride = true;
			return true;
		}
...
void FMetaStoryBindingsChildrenCustomization::CustomizeChildren(IDetailChildrenBuilder& ChildrenBuilder, TSharedPtr<IPropertyHandle> InPropertyHandle)
{
	...
	if (OuterObjects.Num() == 1)
	{
		...
		using FMetaStoryPropertyFunctionNodeProvider = UE::MetaStory::PropertyBinding::FMetaStoryPropertyFunctionNodeProvider;
		UObject* BindingsOwner = UE::MetaStory::PropertyBinding::FindEditorBindingsOwner(OuterObjects[0]);
		if (BindingsOwner && FMetaStoryPropertyFunctionNodeProvider::IsBoundToValidPropertyFunction(*BindingsOwner, TargetPath))
		{
			// Bound PropertyFunction takes control over property's children composition.
			const TSharedPtr<FMetaStoryPropertyFunctionNodeProvider> StructProvider = MakeShared<FMetaStoryPropertyFunctionNodeProvider>(*CastChecked<IMetaStoryEditorPropertyBindingsOwner>(BindingsOwner), MoveTemp(TargetPath));
...
void FMetaStoryBindingExtension::CustomizeDetailWidgetRow(FDetailWidgetRow& InWidgetRow, const IDetailLayoutBuilder& InDetailBuilder, IPropertyBindingBindingCollectionOwner* InBindingsOwner,
...
{
	auto IsClipboardSingleBinding = [](const UE::MetaStoryEditor::FMetaStoryClipboardEditorData& InClipboard)
		{
			...
			TConstArrayView<FMetaStoryPropertyPathBinding> Bindings = InClipboard.GetBindingsInBuffer();
			if (Bindings.IsEmpty())
			{
				return false;
			}
			...
			for (const FMetaStoryPropertyPathBinding& Binding : Bindings)
			{
				if (Binding.GetPropertyFunctionNode().IsValid())
				{
					PropertyFunctionIDs.Add(Binding.GetPropertyFunctionNode().Get<const FMetaStoryEditorNode>().ID);
				}
			}
	...
	auto ImportTextAsSingleBindingClipboard = [IsClipboardSingleBinding, InBindingsOwner, InPropertyHandle](bool bInProcessBuffer, UE::MetaStoryEditor::FMetaStoryClipboardEditorData& OutClipboard)
		{
			TArray<UObject*> OuterObjects;
			InPropertyHandle->GetOuterObjects(OuterObjects);
			...
			UE::MetaStoryEditor::ImportTextAsClipboardEditorData(TargetType, Cast<UMetaStoryEditorData>(InBindingsOwner), UE::MetaStory::PropertyBinding::FindStateOrEditorDataOwner(OuterObjects[0]), OutClipboard, bInProcessBuffer);
	...
	InWidgetRow.AddCustomContextMenuAction(FUIAction(
		FExecuteAction::CreateLambda([InCachedBindingData, SearchMode]()
			{
				using namespace UE::MetaStory::PropertyBinding;
				if (const FMetaStoryCachedBindingData* MetaStoryCachedBindingData = static_cast<FMetaStoryCachedBindingData*>(InCachedBindingData.Get()))
				{
					if (const FPropertyBindingBinding* PropertyBinding = MetaStoryCachedBindingData->FindBinding(SearchMode))
					{
						if (const UMetaStoryEditorData* EditorData = Cast<UMetaStoryEditorData>(MetaStoryCachedBindingData->GetOwner()))
						{
		...
		FExecuteAction::CreateLambda([ImportTextAsSingleBindingClipboard, InCachedBindingData, &InDetailBuilder, SearchMode]()
			{
				...
				if (FMetaStoryCachedBindingData* MetaStoryCachedBindingData = static_cast<FMetaStoryCachedBindingData*>(InCachedBindingData.Get()))
				{
					UMetaStoryEditorData* EditorData = Cast<UMetaStoryEditorData>(MetaStoryCachedBindingData->GetOwner());
					if (!EditorData)
					{
						return;
					}
					...
					{
						TArrayView<FMetaStoryPropertyPathBinding> Bindings = Clipboard.GetBindingsInBuffer();
						check(Bindings.Num());
						...
						FMetaStoryPropertyPathBinding& PropertyBinding = Bindings[0];
						FConstStructView BindingPropertyFunctionNodeView = PropertyBinding.GetPropertyFunctionNode();
						TConstArrayView<TInstancedStruct<FPropertyBindingBindableStructDescriptor>> AccessibleStructs = MetaStoryCachedBindingData->GetAccessibleStructs();
						...
						if (const FMetaStoryEditorNode* PropertyFunctionNode = BindingPropertyFunctionNodeView.GetPtr<const FMetaStoryEditorNode>())
						{
							...
							EditorData->EnumerateBindablePropertyFunctionNodes(
								[PropertyFunctionNodeStruct, &TemplatePropertyFunctionID](const UScriptStruct* NodeStruct, const FMetaStoryBindableStructDesc& Desc, const FMetaStoryDataView Value)
								{
						...
						if (SourceStructIndex == INDEX_NONE)
						{
							...
							EditorData->VisitAllNodes(
								[MetaStoryCachedBindingData, &PropertyBinding, &InaccessibleSourceStructPathName, &TargetStructPathName](const UMetaStoryState* State, const FMetaStoryBindableStructDesc& Desc, const FMetaStoryDataView Value)
								{
									...
									if (MetaStoryCachedBindingData->GetTargetPath().GetStructID() == Desc.ID)
									{
							...
							UE::MetaStoryEditor::AddErrorNotification(
						...
						const FMetaStoryBindableStructDesc& SourceStructDesc = static_cast<const FMetaStoryBindableStructDesc&>(AccessibleStructs[SourceStructIndex].Get());
						const bool bBindToContextStruct = SourceStructDesc.DataSource == EMetaStoryBindableStructSource::Context;
						...
						{
							bIsSourcePropertyCompatible = MetaStoryCachedBindingData->CanBindToContextStruct(SourceStructDesc.Struct, BindingChain[0].ArrayIndex);
						}
						else
						{
							check(SourceProperty);
							bIsSourcePropertyCompatible = MetaStoryCachedBindingData->CanBindToProperty(SourceProperty, BindingChain);
						}
						...
						if (!bIsSourcePropertyCompatible)
						{
							...
							if (bBindToPropertyFunctionNode)
							{
								...
								EditorData->EnumerateBindablePropertyFunctionNodes(
									[&SourceStructName, &SourceStructDesc](const UScriptStruct* NodeStruct, const FMetaStoryBindableStructDesc& Desc, const FMetaStoryDataView Value)
									{
							...
							UE::MetaStoryEditor::AddErrorNotification(FText::Format(
								LOCTEXT("IncompatibleSourceProperty", "Source Property {0} is not compatible with Target Property {1}."),
								SourceText,
								MetaStoryCachedBindingData->GetPropertyHandle()->GetPropertyDisplayName()));

							return;
						}

						// Add/Replace the binding to property
						FScopedTransaction Transaction(LOCTEXT("MetaStoryBindingData_PasteBinding", "Paste Binding"));

						EditorData->Modify();

						MetaStoryCachedBindingData->RemoveBinding(SearchMode);

						// Update the target path to be the current property
						PropertyBinding.GetMutableTargetPath() = MetaStoryCachedBindingData->GetTargetPath();
						...
						PropertyBinding.SetIsOutputBinding(MetaStoryCachedBindingData->HierarchicalPropertyUsage == EMetaStoryPropertyUsage::Output);
						
						for (int32 Idx = 0; Idx < Bindings.Num(); ++Idx)
						{
							FMetaStoryPropertyPathBinding& CurrentBinding = Bindings[Idx];
...
bool FMetaStoryBindingsChildrenCustomization::ShouldCustomizeChildren(TSharedRef<IPropertyHandle> InPropertyHandle)
{
	TArray<UObject*> OuterObjects;
	InPropertyHandle->GetOuterObjects(OuterObjects);
	if (OuterObjects.Num() == 1)
	{
		...
		FPropertyBindingPath TargetPath;
		UE::PropertyBinding::MakeStructPropertyPathFromPropertyHandle(InPropertyHandle, TargetPath);
		IMetaStoryEditorPropertyBindingsOwner* BindingOwner = Cast<IMetaStoryEditorPropertyBindingsOwner>(UE::MetaStory::PropertyBinding::FindEditorBindingsOwner(OuterObjects[0]));
		if (!TargetPath.IsPathEmpty() && BindingOwner)
		{
			if (const FMetaStoryEditorPropertyBindings* EditorBindings = BindingOwner->GetPropertyEditorBindings())
			{
