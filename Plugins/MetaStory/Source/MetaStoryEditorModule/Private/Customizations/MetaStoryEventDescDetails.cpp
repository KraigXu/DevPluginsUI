...
#include "MetaStoryEventDescDetails.h"
...
/** 
 * Node builder for MetaStoryEvent Payload.
 * Draws recursively all children of the selected Payload type without possibility of modifying them.
 */
class FMetaStoryEventPayloadDetails : public IDetailCustomNodeBuilder
{
public:
	using FVisitedStructs = TArray<UStruct*, TInlineAllocator<16>>;

	FMetaStoryEventPayloadDetails(const FProperty& Property, const FVisitedStructs& InVisitedStructs)
	...
	FMetaStoryEventPayloadDetails(FName InName, TSharedRef<SWidget> InNameWidget, TSharedRef<SWidget> InValueWidget, UStruct* InStruct, const FVisitedStructs& InVisitedStructs)
		: Name(InName)
	...
	virtual void GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder) override
	{
		for (TFieldIterator<const FProperty> PropIt(Struct); PropIt; ++PropIt)
		{
			...
			{
				ChildrenBuilder.AddCustomBuilder(MakeShared<FMetaStoryEventPayloadDetails>(Property, VisitedStructs));
			}
		}
	}
...
TSharedRef<IPropertyTypeCustomization> FMetaStoryEventDescDetails::MakeInstance()
{
	return MakeShared<FMetaStoryEventDescDetails>();
}

void FMetaStoryEventDescDetails::CustomizeHeader(TSharedRef<IPropertyHandle> PropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	HeaderRow
	.NameContent()
	[
...
void FMetaStoryEventDescDetails::CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	TSharedPtr<IPropertyHandle> TagProperty = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FMetaStoryEventDesc, Tag));
	check(TagProperty);
	ChildBuilder.AddProperty(TagProperty.ToSharedRef());

	TSharedPtr<IPropertyHandle> PayloadProperty = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FMetaStoryEventDesc, PayloadStruct));
	...
	if (PayloadProperty->GetValue(PayloadType))
	{
		...
		ChildBuilder.AddCustomBuilder(MakeShared<FMetaStoryEventPayloadDetails>(TEXT("EventPayload"), PayloadProperty->CreatePropertyNameWidget(), PayloadProperty->CreatePropertyValueWidget(), PayloadStruct, FMetaStoryEventPayloadDetails::FVisitedStructs()));
	}
	...
	TSharedPtr<IPropertyHandle> ConsumeEventOnSelectProperty = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FMetaStoryEventDesc, bConsumeEventOnSelect));
