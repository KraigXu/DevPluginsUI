...
FMetaStoryEditorCommands::FMetaStoryEditorCommands() 
	: TCommands(
		TEXT("MetaStoryEditor"), // Context name for fast lookup
		LOCTEXT("MetaStoryEditor", "MetaStory Editor"), // Localized context name for displaying
		NAME_None,
		TEXT("MetaStoryEditorStyle")
	)
{
}

void FMetaStoryEditorCommands::RegisterCommands()
{
	UI_COMMAND(Compile, "Compile", "Compile the current MetaStory.", EUserInterfaceActionType::Button, FInputChord(EKeys::F7));
	UI_COMMAND(SaveOnCompile_Never, "Never", "Sets the save-on-compile option to 'Never', meaning that your MetaStory will not be saved when they are compiled", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(SaveOnCompile_SuccessOnly, "On Success Only", "Sets the save-on-compile option to 'Success Only', meaning that your MetaStory will be saved whenever they are successfully compiled", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(SaveOnCompile_Always, "Always", "Sets the save-on-compile option to 'Always', meaning that your MetaStory will be saved whenever they are compiled (even if there were errors)", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(LogCompilationResult, "Log compilation result", "After a MetaStory compiles, log the internal content of the MetaStory.", EUserInterfaceActionType::ToggleButton, FInputChord());
	UI_COMMAND(LogDependencies, "Log dependencies", "After a MetaStory compiles, log the compilation dependencies of the MetaStory.", EUserInterfaceActionType::ToggleButton, FInputChord());
