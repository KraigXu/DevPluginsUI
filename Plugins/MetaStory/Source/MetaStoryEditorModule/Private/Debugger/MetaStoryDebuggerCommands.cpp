...
FMetaStoryDebuggerCommands::FMetaStoryDebuggerCommands()
	: TCommands
	(
		"MetaStoryEditor.Debugger",								// Context name for fast lookup
		LOCTEXT("MetaStoryDebugger", "MetaStory Debugger"),		// Localized context name for displaying
		NAME_None,												// Parent context name
		FMetaStoryEditorStyle::Get().GetStyleSetName()
	)
{
}

void FMetaStoryDebuggerCommands::RegisterCommands()
{
	...
	UI_COMMAND(ResumeDebuggerAnalysis, "Resume Analysis", "Resume trace analysis after hitting breakpoint", EUserInterfaceActionType::Button, FInputChord());
	...
	UI_COMMAND(OpenRewindDebugger, "Open RewindDebugger", "Open RewindDebugger window and its details tab", EUserInterfaceActionType::Button, FInputChord());
}
