...
#define LOCTEXT_NAMESPACE "SMetaStoryDebuggerEventTimelineView"


void SMetaStoryDebuggerEventTimelineView::Construct(const FArguments& InArgs)
{
...
int32 SMetaStoryDebuggerEventTimelineView::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, const int32 LayerId, const FWidgetStyle& InWidgetStyle, const bool bParentEnabled) const
{
...
FVector2D SMetaStoryDebuggerEventTimelineView::ComputeDesiredSize(float) const
{
	return DesiredSize.Get();
}

int32 SMetaStoryDebuggerEventTimelineView::PaintEvents(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (const TSharedPtr<FTimelineEventData> TimelineEventData = EventData.Get())
	{
		...
		if (NumWindows > 0)
		{
			for (int i = 0; i < NumWindows; i++)
			{
				...
				// Ensure at least 1 pixel remains after horizontal size reduction, else don't reduce size
				const float XDelta = (XEnd - XStart);
				const float Width = XDelta - 1 > HSizeReduction ? XDelta - HSizeReduction : XDelta;
				FSlateDrawElement::MakeBox
				(OutDrawElements,
					LayerId++,
					AllottedGeometry.ToPaintGeometry(FVector2f(Width, EventSize.Y - VSizeReduction), FSlateLayoutTransform(FVector2f(XStart, Y + VerticalOffset))),
					Brush,
				...
				if (Window.Description.IsEmpty() == false)
				{
					...
					const int32 HorizontalOffset = FMath::RoundToInt(XDelta - TextLeftOffset);
