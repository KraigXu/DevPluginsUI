...
#include "Debugger/SMetaStoryDebuggerTimelines.h"
...
//----------------------------------------------------------------------//
// SMetaStoryInstanceTimelines
//----------------------------------------------------------------------//
SMetaStoryDebuggerTimelines::SMetaStoryDebuggerTimelines() 
	: SSimpleTimeSlider()
{ 
}
...
void SMetaStoryDebuggerTimelines::SetSelection(const TSharedPtr<RewindDebugger::FRewindDebuggerTrack>& SelectedItem)
{
	InstanceTreeView->SetSelection(SelectedItem);
}

void SMetaStoryDebuggerTimelines::ScrollTo(const double ScrollOffset)
{
	InstanceTreeView->SetScrollOffset(static_cast<float>(ScrollOffset));
}

TSharedRef<ITableRow> SMetaStoryDebuggerTimelines::GenerateTreeRow(TSharedPtr<RewindDebugger::FRewindDebuggerTrack> InItem, const TSharedRef<STableViewBase>& OwnerTable)
{
...
void SMetaStoryDebuggerTimelines::TreeExpansionChanged(TSharedPtr<RewindDebugger::FRewindDebuggerTrack> InItem, bool bShouldBeExpanded)
{
	InItem->SetIsExpanded(bShouldBeExpanded);
	OnExpansionChanged.ExecuteIfBound();
}

void SMetaStoryDebuggerTimelines::Construct(const FArguments& InArgs)
{
	...
	InstanceTreeView = SNew(STreeView<TSharedPtr<RewindDebugger::FRewindDebuggerTrack>>)
									...
									.OnGenerateRow(this, &SMetaStoryDebuggerTimelines::GenerateTreeRow)
									.OnGetChildren_Static(&UE::MetaStoryDebugger::TimelineViewGetChildren)
									.OnExpansionChanged(this, &SMetaStoryDebuggerTimelines::TreeExpansionChanged)
...
int32 SMetaStoryDebuggerTimelines::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
...
FVector2D SMetaStoryDebuggerTimelines::ComputeDesiredSize(float X) const
{
...
void SMetaStoryDebuggerTimelines::RestoreExpansion()
{
	for (const auto& Track : *InstanceTracks)
	{
		UE::MetaStoryDebugger::RestoreTimelineExpansion(Track, InstanceTreeView);
	}
}

void SMetaStoryDebuggerTimelines::Refresh()
{
