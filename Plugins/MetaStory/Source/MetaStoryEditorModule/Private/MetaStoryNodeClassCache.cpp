...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MetaStoryNodeClassCache)

#define LOCTEXT_NAMESPACE "MetaStoryEditor"

FMetaStoryNodeClassData::FMetaStoryNodeClassData(TNotNull<UStruct*> InStruct)
...
// deprecated
FMetaStoryNodeClassData::FMetaStoryNodeClassData(const FString& InClassAssetName, const FString& InClassPackageName, const FName InStructName, UStruct* InStruct)
	: StructAssetPath(*InClassPackageName, InStructName)
	, CachedStruct(InStruct)
{
}

FMetaStoryNodeClassData::FMetaStoryNodeClassData(const FTopLevelAssetPath& AssetPath)
...
UStruct* FMetaStoryNodeClassData::GetStruct(bool bSilent)
{
...
const UStruct* FMetaStoryNodeClassData::GetInstanceDataStruct(bool bSilent /*= false*/)
{
	...
	if (Result == nullptr && StructAssetPath.IsValid())
	{
		...
		{
			TInstancedStruct<FMetaStoryNodeBase> NodeInstance;
...
//----------------------------------------------------------------------//
//  FMetaStoryNodeClassCache
//----------------------------------------------------------------------//
FMetaStoryNodeClassCache::FMetaStoryNodeClassCache()
{
	// Register with the Asset Registry to be informed when it is done loading up files.
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::GetModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	AssetRegistryModule.Get().OnFilesLoaded().AddRaw(this, &FMetaStoryNodeClassCache::InvalidateCache);
	AssetRegistryModule.Get().OnAssetAdded().AddRaw(this, &FMetaStoryNodeClassCache::OnAssetAdded);
	AssetRegistryModule.Get().OnAssetRemoved().AddRaw(this, &FMetaStoryNodeClassCache::OnAssetRemoved);

	// Register to have Populate called when doing a Reload.
	FCoreUObjectDelegates::ReloadCompleteDelegate.AddRaw(this, &FMetaStoryNodeClassCache::OnReloadComplete);

	// Register to have Populate called when a Blueprint is compiled.
	if (GIsEditor)
	{
		check(GEditor);
		GEditor->OnBlueprintCompiled().AddRaw(this, &FMetaStoryNodeClassCache::InvalidateCache);
		GEditor->OnClassPackageLoadedOrUnloaded().AddRaw(this, &FMetaStoryNodeClassCache::InvalidateCache);
	}
}
...
void FMetaStoryNodeClassCache::AddRootStruct(TNotNull<UStruct*> RootStruct)
{
...
void FMetaStoryNodeClassCache::GetStructs(TNotNull<const UStruct*> BaseStruct, TArray<TSharedPtr<FMetaStoryNodeClassData>>& AvailableClasses)
{
...
void FMetaStoryNodeClassCache::OnAssetAdded(const FAssetData& AssetData)
{
	UpdateBlueprintClass(AssetData);
}

void FMetaStoryNodeClassCache::OnAssetRemoved(const FAssetData& AssetData)
{
	...
	if (AssetData.GetTagValue(FBlueprintTags::GeneratedClassPath, AssetClassName) && AssetData.GetTagValue(FBlueprintTags::NativeParentClassPath, AssetNativeParentClassName))
	{
		if (const int32* RootClassIndex = ClassNameToRootIndex.Find(AssetNativeParentClassName))
		{
			...
			RootClass.ClassData.RemoveAll([&StructAssetPath](const TSharedPtr<FMetaStoryNodeClassData>& ClassData)
			{
...
void FMetaStoryNodeClassCache::InvalidateCache()
{
...
void FMetaStoryNodeClassCache::OnReloadComplete(EReloadCompleteReason Reason)
{
	InvalidateCache();
}

void FMetaStoryNodeClassCache::UpdateBlueprintClass(const FAssetData& AssetData)
{
	FString AssetClassName;
	FString AssetNativeParentClassName;
	if (AssetData.GetTagValue(FBlueprintTags::GeneratedClassPath, AssetClassName) && AssetData.GetTagValue(FBlueprintTags::NativeParentClassPath, AssetNativeParentClassName))
	{
		...
		if (const int32* RootClassIndex = ClassNameToRootIndex.Find(AssetNativeParentClassName))
		{
			...
			{
				FRootClassContainer& RootClass = RootClasses[*RootClassIndex];
				const int32 ClassDataIndex = RootClass.ClassData.IndexOfByPredicate([&StructAssetPath](const TSharedPtr<FMetaStoryNodeClassData>& ClassData)
					{
						return ClassData->GetStructPath() == StructAssetPath;
					});
				...
				if (ClassDataIndex == INDEX_NONE)
				{
					...
					if (AssetClass)
					{
						TSharedRef<FMetaStoryNodeClassData> NewData = MakeShared<FMetaStoryNodeClassData>(AssetClass);
						RootClass.ClassData.Add(NewData);
					}
					else
					{
						TSharedRef<FMetaStoryNodeClassData> NewData = MakeShared<FMetaStoryNodeClassData>(StructAssetPath);
						RootClass.ClassData.Add(NewData);
					}
				}
			}
...
void FMetaStoryNodeClassCache::CacheClasses()
{
	for (TEnumerateRef<FRootClassContainer> RootClass : EnumerateRange(RootClasses))
	{
		...
		if (const UClass* Class = Cast<UClass>(RootClass->BaseStruct.Get()))
		{
			for (TObjectIterator<UClass> It; It; ++It)
			{
				...
				{
					RootClass->ClassData.Add(MakeShareable(new FMetaStoryNodeClassData(TestClass)));
		...
		else if (const UScriptStruct* ScriptStruct = Cast<UScriptStruct>(RootClass->BaseStruct.Get()))
		{
			for (TObjectIterator<UScriptStruct> It; It; ++It)
			{
				...
				{
					RootClass->ClassData.Add(MakeShareable(new FMetaStoryNodeClassData(TestStruct)));
