...
struct FMetaStoryTest_TasksCompletion_AllAny : FMetaStoryTestBase
{
	virtual bool InstantTest() override
	{
		...
		UMetaStoryState* RootState = nullptr;
		{
			...
			UMetaStoryState& Root = EditorData.AddSubTree(FName(TEXT("Tree1StateRoot")));
			RootState = &Root;
			...
			for (int32 TaskIndex = 0; TaskIndex < FMetaStoryTasksCompletionStatus::MaxNumberOfTasksPerGroup; ++TaskIndex)
			{
				...
				if (TaskIndex == 10)
				{
					...
					Task.GetNode().TickCompletionResult = EMetaStoryRunStatus::Failed;
				}
			}
			UMetaStoryState& Tree1StateA = Root.AddChildState("Tree1StateA");
			...
			for (int32 TaskIndex = 0; TaskIndex < FMetaStoryTasksCompletionStatus::MaxNumberOfTasksPerGroup; ++TaskIndex)
			{
				...
				{
					Task.GetNode().bConsideredForCompletion = false;
					Task.GetNode().EnterStateResult = EMetaStoryRunStatus::Failed;
				}
			}
		...
		// Test All
		{
			...
			for (int32 TickIndex = 0; TickIndex < FMetaStoryTasksCompletionStatus::MaxNumberOfTasksPerGroup; ++TickIndex)
			{
				...
				const bool bLastTick = TickIndex == FMetaStoryTasksCompletionStatus::MaxNumberOfTasksPerGroup - 1;
		...
		// Test any
		RootState->TasksCompletion = EMetaStoryTaskCompletionType::Any;
		{
			FMetaStoryCompilerLog Log;
			FMetaStoryCompiler Compiler(Log);
			const bool bResult = Compiler.Compile(MetaStory);
...
struct FMetaStoryTest_TasksCompletion_Status : FMetaStoryTestBase
{
	virtual bool InstantTest() override
	{
		FMetaStoryCompactFrame Frame;
		Frame.NumberOfTasksStatusMasks = 1;
		FMetaStoryTasksCompletionStatus Status = FMetaStoryTasksCompletionStatus(Frame);

		constexpr int32 NumberOfTasks = 4;
		FMetaStoryCompactState State;
		State.CompletionTasksControl = EMetaStoryTaskCompletionType::All;
		...
		UE::MetaStory::FTasksCompletionStatus TestingStatus = Status.GetStatus(State);
		...
		auto TestEmpty = [this, &TestingStatus]()
			{
				...
				AITEST_EQUAL(TEXT("The completion status is running."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Running);
				for (int32 Index = 0; Index < NumberOfTasks; ++Index)
				{
					...
					AITEST_EQUAL(TEXT("Empty is running."), TestingStatus.GetStatus(Index), UE::MetaStory::ETaskCompletionStatus::Running);
				}
				return true;
			};
		...
		{
			TestingStatus.SetStatus(1, UE::MetaStory::ETaskCompletionStatus::Running);
			if (!TestEmpty())
			{
				return false;
			}
		...
		auto Test1Expected = [this, &TestingStatus](UE::MetaStory::ETaskCompletionStatus Expected)
			{
				...
				AITEST_EQUAL(TEXT("States running."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Running);
				for (int32 Index = 0; Index < NumberOfTasks; ++Index)
				{
					...
					else
					{
						...
						AITEST_EQUAL(TEXT("1 is Stopped."), TestingStatus.GetStatus(Index), UE::MetaStory::ETaskCompletionStatus::Running);
					}
				}
				return true;
			};
		// Set tasks (1) to stop.
		{
			TestingStatus.SetStatus(1, UE::MetaStory::ETaskCompletionStatus::Stopped);
			if (!Test1Expected(UE::MetaStory::ETaskCompletionStatus::Stopped))
			{
				return false;
			}
		}
		// Set tasks (1) to running. Does nothing.
		{
			ETaskCompletionStatus NewStatus = TestingStatus.SetStatusWithPriority(1, UE::MetaStory::ETaskCompletionStatus::Running);
			AITEST_EQUAL(TEXT("1 make the state Stopped."), NewStatus, UE::MetaStory::ETaskCompletionStatus::Stopped);
			if (!Test1Expected(UE::MetaStory::ETaskCompletionStatus::Stopped))
			{
				return false;
			}
		...
		{
			ETaskCompletionStatus NewStatus = TestingStatus.SetStatusWithPriority(1, UE::MetaStory::ETaskCompletionStatus::Succeeded);
			AITEST_EQUAL(TEXT("1 make the state Succeed."), NewStatus, UE::MetaStory::ETaskCompletionStatus::Succeeded);
			if (!Test1Expected(UE::MetaStory::ETaskCompletionStatus::Succeeded))
			{
				return false;
			}
		}
		...
		{
			ETaskCompletionStatus NewStatus = TestingStatus.SetStatusWithPriority(1, UE::MetaStory::ETaskCompletionStatus::Stopped);
			AITEST_EQUAL(TEXT("1 make the state Succeed."), NewStatus, UE::MetaStory::ETaskCompletionStatus::Succeeded);
			if (!Test1Expected(UE::MetaStory::ETaskCompletionStatus::Succeeded))
			{
				return false;
		...
		{
			ETaskCompletionStatus NewStatus = TestingStatus.SetStatusWithPriority(1, UE::MetaStory::ETaskCompletionStatus::Failed);
			AITEST_EQUAL(TEXT("1 make the state Succeed."), NewStatus, UE::MetaStory::ETaskCompletionStatus::Failed);
			...
			AITEST_EQUAL(TEXT("States failed."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Failed);
			for (int32 Index = 0; Index < NumberOfTasks; ++Index)
			{
				if (1 == Index)
				{
					...
					AITEST_EQUAL(TEXT("1 is failed."), TestingStatus.GetStatus(Index), UE::MetaStory::ETaskCompletionStatus::Failed);
				}
				else
				{
					AITEST_FALSE(TEXT("1 is not completed."), TestingStatus.HasFailed(Index));
					AITEST_TRUE(TEXT("1 is running."), TestingStatus.IsRunning(Index));
					AITEST_EQUAL(TEXT("1 is running."), TestingStatus.GetStatus(Index), UE::MetaStory::ETaskCompletionStatus::Running);
				}
			}
		}
		// Set completion status
		{
			TestingStatus.SetCompletionStatus(UE::MetaStory::ETaskCompletionStatus::Stopped);
			...
			AITEST_EQUAL(TEXT("States stopped."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Stopped);
			for (int32 Index = 0; Index < NumberOfTasks; ++Index)
			{
				AITEST_FALSE(TEXT("1 is not completed."), TestingStatus.HasFailed(Index));
				AITEST_FALSE(TEXT("1 is stopped."), TestingStatus.IsRunning(Index));
				AITEST_EQUAL(TEXT("1 is stopped."), TestingStatus.GetStatus(Index), UE::MetaStory::ETaskCompletionStatus::Stopped);
			}
		}
		// Test GetCompletionStatus
		{
			TestingStatus.SetCompletionStatus(UE::MetaStory::ETaskCompletionStatus::Running);
			AITEST_EQUAL(TEXT("States stopped."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Running);
		}
		// Test GetCompletionStatus
		{
			TestingStatus.SetCompletionStatus(UE::MetaStory::ETaskCompletionStatus::Succeeded);
			AITEST_EQUAL(TEXT("States succeeded."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Succeeded);
		}
		// Test GetCompletionStatus
		{
			TestingStatus.SetCompletionStatus(UE::MetaStory::ETaskCompletionStatus::Failed);
			AITEST_EQUAL(TEXT("States failed."), TestingStatus.GetCompletionStatus(), UE::MetaStory::ETaskCompletionStatus::Failed);
		}
