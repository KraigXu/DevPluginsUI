...
- [~] 独立编辑器窗口骨架已实现（Node List / Asset List / Main / Details 四区域）。**Main** 已接入主视图画布（`SMetaplotFlowGraphWidget`：科技树风格网格、节点与连线可视化、中键平移/滚轮缩放/右下角鹰眼，并与 Node List 选中同步）。**Asset List** 仍为占位；节点/连线的主要编辑入口仍在 Node List 与 Details。

#### B. 编辑器目标功能（第 5 章）

- [~] 节点创建/删除/属性编辑已具备最小可用能力（Node List 可增删，属性可在 Details 编辑）；复制/粘贴与完整图节点语义未实现。
- [~] 连线在资产与主视图中可读（`Transitions` 数据 + Main 区贝塞尔绘制）；**画布上拖拽引脚建线、实时环检测与拒绝**未实现（连线仍通过 Node List「+ 连线」等操作维护）。
...
- [ ] 实时校验（环、孤立节点、无效行为等）未实现。
- [ ] 编辑器内模拟运行模式未实现。
- [ ] 游戏内调试命令/日志体系未实现。
- [ ] 网络复制、Server/Multicast RPC、多人实例策略未实现。

> 备注：Runtime 与「画布上建线 / 模拟运行」等仍为规划能力；编辑器侧已具备 **Main 流程画布可视化** 与基础资产编辑，整体仍偏「可编辑脚手架」而非完整第 5 章编辑器。
...
目标结构示意如下；**当前工程**为四区域：**Node List**（左列上）、**Asset List**（左列下）、**Main**（中央图表）、**Details**（右列）。中央 **Main** 已对应下图中的图表视图能力（网格、节点、连线、鹰眼），尚未实现独立「节点模板库」侧栏与顶部工具栏条目。
...
**实现进度（代码，简要）：** Main 区已按 `StageIndex` / `LayerIndex` 绘制网格与节点卡片（宽约 160px），连线为浅色贝塞尔路径 + 箭头；中键平移、滚轮缩放（约 20%～200%）、右下角鹰眼与视口框已具备。**尚未实现：** 运行时四态着色（§3.2）、悬停信息卡片（§3.2）、画布拖拽建线与环检测交互（§3.3）。
...
- 带箭头的贝塞尔曲线，颜色浅灰。
- **禁止反向连接**，编辑器会在拖拽时实时检测并拒绝形成环。

#### 3.4 画布导航

- 鼠标中键拖拽平移。
- 滚轮缩放（范围约 20% ~ 200%）。
- 鹰眼视图（右下角缩略图，显示当前视口位置）。

#### 3.5 与蓝图编辑器的视觉差异

| 维度 | Metaplot |
|------|----------|
| 更简洁 | 节点无输入/输出引脚，只有整体入口/出口逻辑 |
| 强调流程 | 节点按网格排列，连线清晰 |
